...
RUN git clone https://github.com/robotology/tactile-control/tree/master/tactile-control-lib.git /tactile-control-lib \
 && cd /tactile-control-lib \
 && mkdir build \
 && cd build \
 && cmake  ../ \
 && make -j4 install \
 && rm -fr /tactile-control-lib

##################################### COPY #####################################

RUN mkdir ${HOME}/superquadric-grasp

COPY . ${HOME}/superquadric-grasp

#################################### CMAKE #####################################

RUN mkdir ${HOME}/superquadric-grasp/build \
 && cd ${HOME}/superquadric-grasp/build \
 && cmake  .. \
 && make -j2
...
WORKDIR ${HOME}/superquadric-grasp
